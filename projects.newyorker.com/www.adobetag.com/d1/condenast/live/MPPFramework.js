...
function TagContainer(n) {
        ...
        if (cm) {
            ...
            if (c) {
                ...
                while (!cp && i < cm.length) {
                    ...
                    if (ci >= 0)
                        if (cm[i].v) {
                            cv = c.substring(ci + cm[i].k.length + 2);
                            ci = cv.indexOf(';');
                            if (ci >= 0) {
                                cv = unescape(cv.substring(0, ci));
                                if (cv == cm[i].v) cp = 1
                            }
                        } else cp = 1;
                    i++
    ...
    t.cr = function() {
        ...
        if (t.cl.length > 0) {
            if (!d.body) {
                if (!t.crt) t.crt = setTimeout(function() {
                    t.crt = 0;
                    t.cr()
                }, 13)
            } else {
                        ...
                        if ((p.t == 's' && p.u) || (p.t == 'c' && p.c)) {
                            ...
                            if (p.t == 's') {
                                ...
                                o.i = setInterval(function() {
                                    ...
                                    if (o.readyState == 'complete' || o.t > 2) {
                                        o.c();
                                        o.n()
                                    }
                                }, 50);
...
function s_doPlugins(s) {
    ...
    if (_cn_o.rc('tagEnv').indexOf('detailscom') != -1) {
        s.eVar33 = " ";
        s.eVar33 = ""; // clear value from omniture.js for override
